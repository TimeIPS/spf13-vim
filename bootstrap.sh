...
if [ ! -e $endpath/.git ]; then
    ...
    git clone --recursive -b $branch http://github.com/TimeIPS/spf13-vim.git $endpath
else
...
system_shell=$SHELL
export SHELL="/bin/sh"
vim -X -E -s -u $endpath/.vimrc.bundles +BundleInstall! +BundleClean +qall > /dev/null 2>&1 &
